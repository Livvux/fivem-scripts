...
AddEventHandler('esx_jobChat:911', function(targetCoords, msg, streetName, emergency)
	...
	if emergency == '911' then
		messageFull = {
			template = '<div style="padding: 8px; margin: 8px; background-color: rgba(219, 35, 35, 0.9); border-radius: 25px;"><i class="fas fa-bell"style="font-size:15px"></i> [ENTORNO] | ANONIMO : {0} | Localización : {1} | {2}</font></i></b></div>',
        	args = {fal, streetName, msg}
		}
	else
		messageFull = {
			template = '<div style="padding: 8px; margin: 8px; background-color: rgba(255, 102, 0, 0.9); border-radius: 25px;"><i class="fas fa-bell"style="font-size:15px"></i> [AUXILIO] | ANONIMO : {0} | Localización : {1} | {2}</font></i></b></div>',
        	args = {fal, streetName, msg}
		}
	end
